...
y direcciones de entrega en \textbf{DynamoDB nexo-addresses}, y expone un API
protegido a través de \textbf{Lambda + API Gateway} con autorizador Cognito.
...
\begin{lstlisting}[caption={Arquitectura general del backend}, numbers=none]
  Frontend (Next.js)
       ...
       |-- GET/POST/PUT/DELETE /addresses      |               |
                                               v               v
                      Lambda nexo-admin-users  Lambda nexo-orders-api  Lambda nexo-addresses-api
                               |                       |                       |
                      +--------+--------+    +----------+----------+   +-------+-------+
                      |                 |    |          |          |   |               |
                   Cognito           DynamoDB        DynamoDB   DynamoDB           DynamoDB
                (admin ops)         nexo-users      nexo-orders nexo-orders       nexo-addresses
\end{lstlisting}
...
\begin{lstlisting}[language=bash, caption={Esquema de ítem en nexo-orders}]
{
  ...
  "peso":             number,     // kg, opcional, lo carga el admin
