...
\texttt{/admin} & Protegida (admin) & Dashboard: tabs Usuarios, Pedidos y Reseñas \\
\texttt{/admin/[id]} & Protegida (admin) & Editar o eliminar un usuario \\
\texttt{/resenas} & Público & Galería pública de reseñas de clientes \\
\texttt{/privacidad} & Público & Política de privacidad bilingüe (ES/EN) \\
\texttt{/terminos} & Público & Términos y condiciones bilingüe (ES/EN) \\
\texttt{/reembolsos} & Público & Política de reembolsos bilingüe (ES/EN) \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  ...
  \item \textbf{Reseña de pedido entregado:} Al tener al menos un pedido con estado
    \texttt{entregado}, aparece el botón ``Dejar reseña'' al pie de la lista de pedidos.
    Abre un modal con selector de estrellas (1--5) y campo de comentario (mínimo 10
    caracteres). La lógica se delega a \texttt{createReview()} de
    \texttt{src/lib/reviews.ts}.
  \item \textbf{Restricciones de reseña:} Solo se puede dejar \textbf{una} reseña por
    cuenta. Si el usuario ya dejó reseña, el botón no se muestra nuevamente.
\end{itemize}
...
Reseñas & Todas las reseñas del sistema & \texttt{GET /reviews} (público) \\
\bottomrule
\end{tabular}
...
\begin{nexobox}
\textbf{Tab Reseñas:} Muestra todas las reseñas ordenadas por fecha. Cada fila
muestra el nombre del autor, calificación (1--5 estrellas), comentario y fecha.
El administrador puede eliminar cualquier reseña con botón de borrado y confirmación.
La eliminación llama a \texttt{deleteReview()} de \texttt{src/lib/reviews.ts}.
\end{nexobox}
...
\subsection{Sistema de Reseñas --- src/lib/reviews.ts}
...
\textbf{Función} & \textbf{Descripción} \\
\midrule
\texttt{getReviews()} & GET \texttt{/reviews} --- todas las reseñas (público, sin auth, \texttt{revalidate: 60\,s}) \\
\texttt{createReview(data)} & POST \texttt{/reviews} --- crea reseña (rating + comment, auth requerido) \\
\texttt{deleteReview(reviewId)} & DELETE \texttt{/admin/reviews/\{id\}} --- elimina reseña (solo admin) \\
\bottomrule
\end{tabular}
\caption{API de reseñas --- src/lib/reviews.ts}
\end{table}

\begin{nexobox}
\textbf{Nota de fetch:} \texttt{getReviews()} usa \texttt{next: \{ revalidate: 60 \}}
para ISR en Next.js 14. El endpoint \texttt{GET /reviews} no requiere token de
autenticación: es el único endpoint público del API Gateway.
\end{nexobox}
...
% ==========================================================
\section{Páginas Legales}

\begin{nexobox}
Las tres páginas legales de nexo son \textbf{bilingües} (Español / English). El idioma
activo se selecciona con un toggle en la parte superior de la página, sin recarga
(\texttt{useState}). La última fecha de actualización es Mayo 2026 / May 2026.
\end{nexobox}

\subsection{Componente reutilizable --- src/components/legal/LegalPage.tsx}

\begin{table}[H]
\centering
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Prop} & \textbf{Descripción} \\
\midrule
\texttt{titleEs} / \texttt{titleEn} & Título de la página en cada idioma \\
\texttt{lastUpdated} & Fecha de última actualización (ej.\ ``Mayo 2026 / May 2026'') \\
\texttt{sectionsEs} / \texttt{sectionsEn} & Array de \texttt{\{heading, content\}} por idioma \\
\bottomrule
\end{tabular}
\caption{Props de LegalPage.tsx}
\end{table}

El campo \texttt{content} de cada sección puede ser \texttt{string} (párrafo) o
\texttt{string[]} (lista con viñetas de color cyan). El componente numera las secciones
automáticamente e incluye pie de página con enlace a \texttt{nexxo.courier@gmail.com}
y WhatsApp.

\subsection{Páginas y rutas legales}

\begin{table}[H]
\centering
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Ruta} & \textbf{Título ES} & \textbf{Contenido principal} \\
\midrule
\texttt{/privacidad} & Política de Privacidad & Responsable, datos recogidos, finalidad, derechos ARCO \\
\texttt{/terminos}   & Términos y Condiciones & Casillero, tarifas, artículos prohibidos, responsabilidad \\
\texttt{/reembolsos} & Política de Reembolsos & Elegibilidad, proceso, montos, exclusiones \\
\bottomrule
\end{tabular}
\caption{Páginas legales del sitio}
\end{table}

\begin{nexobox}
\textbf{Acceso:} Las tres páginas son públicas y no requieren autenticación.
Son accesibles desde el \texttt{Footer} del sitio. Ninguna requiere protección de ruta.
\end{nexobox}

% ==========================================================
\section{Página de Reseñas Públicas --- /resenas}

Página pública que muestra todas las reseñas de clientes que recibieron paquetes con
nexo. No requiere autenticación.

\begin{itemize}
  \item Carga las reseñas con \texttt{getReviews()} al montar el componente
    (\texttt{useEffect}).
  \item Muestra el conteo total de reseñas en el subtítulo.
  \item Durante la carga muestra 4 \textbf{skeleton cards} animadas (\texttt{animate-pulse}).
  \item Cada \texttt{ReviewCard} muestra: nombre del autor, fecha formateada en
    \texttt{es-CR}, badge ``\checkmark{} Paquete entregado'', estrellas y comentario.
  \item Si no hay reseñas, muestra estado vacío con CTA para que el usuario deje la
    primera reseña desde \texttt{/pedidos}.
  \item Botón ``Volver al inicio'' (\texttt{ArrowLeft}) para navegar a \texttt{/}.
\end{itemize}

% ==========================================================
\section{Página Nosotros --- /nosotros}

Página institucional pública. Construida como Server Component (sin directiva
\texttt{'use client'}). Compone secciones reutilizables del landing:

\begin{itemize}
  \item \textbf{Hero:} Título ``Ticos que entienden al tico'', texto sobre el origen de la empresa.
  \item \textbf{Misión y Visión:} Dos cards lado a lado (\texttt{md:grid-cols-2}).
  \item \textbf{Historia:} Narrativa de fundación con visual placeholder USA\,$\rightarrow$\,CR.
  \item \textbf{ValuesSection:} Sección de valores reutilizada del landing.
  \item \textbf{ContactSection:} Datos de contacto.
  \item \textbf{CtaBannerSection:} Llamada a cotizar.
\end{itemize}
...
\begin{description}
  ...
  \item[\textbf{ISR}] Incremental Static Regeneration. Estrategia de Next.js que
    regenera páginas estáticas en el servidor con una frecuencia configurable
    (\texttt{revalidate}). Usada en \texttt{getReviews()} para refrescar la caché
    cada 60 segundos.
  \item[\textbf{JWT}] JSON Web Token. Formato de token firmado que Cognito emite
    tras la autenticación exitosa.
  \item[\textbf{LegalPage}] Componente React reutilizable que renderiza páginas
    legales bilingües con toggle ES/EN, numeración automática de secciones y
    soporte para contenido tipo párrafo o lista.
  ...
  \item[\textbf{Seed data}] Datos de ejemplo insertados automáticamente al hacer
    deploy. En nexo se usan 3 reseñas de demostración con \texttt{userId = "seed"}
    creadas vía CDK Custom Resource.
